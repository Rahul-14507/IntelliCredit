...
async def run_research(
    ...
    existing = await db.execute(
        select(Analysis).where(Analysis.entity_id == entity_id)
    )
...
async def recalculate(entity_id: str, db: AsyncSession = Depends(get_db)):
    existing = await db.execute(select(Analysis).where(Analysis.entity_id == entity_id))
    analysis = existing.scalar_one_or_none()
    if analysis:
        await db.delete(analysis)
        await db.commit()
    return {"status": "cleared", "message": "POST /analysis/{id}/research to restart"}
