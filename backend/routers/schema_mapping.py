...
async def classify_all(entity_id: str, db: AsyncSession = Depends(get_db)):
    ...
    for doc in docs:
        try:
            ...
            # Pre-load default schema
            doc.schema_fields = copy.deepcopy(
                DEFAULT_SCHEMAS.get(cls["predicted_type"], [])
            )
            classified.append({
                "doc_id":     doc.id,
...
# ── 3A: Human confirms / overrides single doc ────────────────────────────
...
async def confirm_classification(
    ...
    if payload.action == "reject":
        ...
        # Reload schema if type was changed
        if payload.confirmed_type != doc.predicted_doc_type:
            doc.schema_fields = copy.deepcopy(
                DEFAULT_SCHEMAS.get(payload.confirmed_type, [])
            )
...
# ── 3B: Update schema fields for a document ─────────────────────────────
class SchemaUpdatePayload(BaseModel):
    schema_fields: list  # array of {key, label, type, include}
...
# ── 3C: Run structured extraction on all confirmed docs ──────────────────
