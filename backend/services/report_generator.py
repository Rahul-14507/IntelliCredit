...
def generate_word_report(entity, analysis, docs, output_path: str):
    ...
    values  = [
        ...
        analysis.grade or "",
        f"₹{analysis.recommended_limit_cr} Cr" if analysis.recommended_limit_cr else "",
    ...
    if analysis.scores:
        ...
        score_table.cell(0, 2).text = "Rationale"
        rationale = analysis.scores.get("rationale", {}) if isinstance(analysis.scores, dict) else {}
        for k, v in analysis.scores.items():
            if k != "overall" and k != "rationale":
    ...
    if analysis.reasoning_chain:
        for step in analysis.reasoning_chain:
            ...
            doc.add_paragraph(f"Signal: {step.get('signal')}  |  Impact: {step.get('impact')}  |  Weight: {step.get('weight')}")
    ...
    if analysis.sentiment:
        ...
        doc.add_paragraph(f"Overall Sentiment: {s.get('overall_sentiment')} (score: {s.get('sentiment_score')})")
        if s.get("red_flags"):
